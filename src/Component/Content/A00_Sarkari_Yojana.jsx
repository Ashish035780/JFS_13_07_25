...
import img from "./Pic/Sarkari_yojana.jpg";
...
const App = () => {
  ...
  useEffect(() => {
    fetch("https://raw.githubusercontent.com/Ashishbhagatraj/JFS_Database/refs/heads/main/AAA_Config.json") 
      .then((res) => res.json())
      .then((data) => {
        const newJobData = data.find((item) => item.id === "10th");
  ...
  return (
    <div className={style.aaa}>
      ...
      <div className={style.abch}>
        <h1 className={style.abci}>10th/12th </h1>
       {jobs.map((job, index) => (<A00_Sarkari_Yojana key={index} job={job} />))}
    </div>
        </div>
           <Ad type="468x60" />
