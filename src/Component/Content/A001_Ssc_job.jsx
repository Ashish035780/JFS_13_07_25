...
import img from "./Pic/Ssc_Job.jpg";
...
const App = () => {
  ...
  useEffect(() => {
    fetch("https://raw.githubusercontent.com/Ashishbhagatraj/JFS_Database/refs/heads/main/AAA_Config.json") 
      .then((res) => res.json())
      .then((data) => {
        const newJobData = data.find((item) => item.id === "sscjob");
  ...
  return (
    <div className={style.aaa}>
      ...
      <div className={style.abch}>
        <h1 className={style.abci}>SSC Jobs</h1>
       {jobs.map((job, index) => (<A001_Ssc_job key={index} job={job} />))}
    </div>
        </div>
           <Ad type="468x60" />
