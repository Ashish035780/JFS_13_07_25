...
import img from "./Pic/Admission_Form.jpg";
...
const App = () => {
  ...
  useEffect(() => {
    fetch("https://raw.githubusercontent.com/Ashishbhagatraj/JFS_Database/refs/heads/main/AAA_Config.json") 
      .then((res) => res.json())
      .then((data) => {
        const newJobData = data.find((item) => item.id === "admission");
  ...
  return (
    <div className={style.aaa}>
      ...
      <div className={style.abch}>
        <h1 className={style.abci}>Admission Form</h1>
       {jobs.map((job, index) => (<A00_Admission_form key={index} job={job} />))}
    </div>
        </div>
           <Ad type="468x60" />
