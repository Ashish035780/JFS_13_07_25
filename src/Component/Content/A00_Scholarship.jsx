...
import img from "./Pic/Scholarship.jpg";
...
const App = () => {
  ...
  useEffect(() => {
    fetch("https://raw.githubusercontent.com/Ashishbhagatraj/JFS_Database/refs/heads/main/AAA_Config.json") 
      .then((res) => res.json())
      .then((data) => {
        const newJobData = data.find((item) => item.id === "diploma");
  ...
  return (
    <div className={style.aaa}>
      ...
      <div className={style.abch}>
        <h1 className={style.abci}>Diploma</h1>
       {jobs.map((job, index) => (<A00_Scholarship key={index} job={job} />))}
    </div>
        </div>
           <Ad type="468x60" />
